...
void task_interface_analysis_report( TaskDependencies& dependencies )
{
    ...
    boost::filesystem::path reportFilePath
        = "/src/meta/interface_report.html";
    ...
    for( auto pInterface : database.many< Interface >( src ) )
    {
        ...
        for( auto pFunction : pInterface->get_functions() )
        {
            ...
            row.push_back(
                L{ qualTypeToStr( pFunction->get_return_type() ) } );
            ...
            for( auto pParam : pFunction->get_parameters() )
            {
                ...
                param.push_back( L{
                    qualTypeToStr( pParam->get_qualified_type() ) } );
                param.push_back( L{ pParam->get_name() } );
                param.push_back(
                    L{ pQualifiedType->get_is_const() } );
                param.push_back(
                    L{ pQualifiedType->get_is_pointer() } );
                param.push_back(
                    L{ pQualifiedType->get_is_reference() } );
                param.push_back(
                    L{ pQualifiedType->get_is_rvalue() } );
                param.push_back( L{ pQualifiedType->get_name() } );
                params.push_back( param );
            }
            row.push_back(
                T{ { "Type", "Name", "Const", "Pointer", "Reference",
                     "RValue", "Underlying Type" },
                   params } );
        ...
        auto r = B{
            ...
            { T{ { "Function Name"s, "Return Type"s, "Parameters"s },
                 rows } } };

        if( auto pFactory = db_cast< Factory >( pInterface ) )
        {
            std::cout << "Report found factory: "
                      << pInterface->get_full_type_name()
                      << std::endl;
        ...
        {
            std::cout << "Report found interface: "
                      << pInterface->get_full_type_name()
                      << std::endl;
    ...
    Cont report = B{ { "Megastructure Service Interface Report"s },
    ...
    auto pFile
        = boost::filesystem::createNewFileStream( reportFilePath );
    *pFile << os.str();
