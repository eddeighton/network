...
#define VERIFY_RTE(msg)
#define VERIFY_RTE_MSG(expr, msg)
#define THROW_RTE(msg) std::ostringstream _os; _os << msg; throw std::runtime_error(_os.str())
...
void loadShader( const std::string& filePath, std::vector< std::uint32_t >& shaderByteCode )
{
    std::ifstream inputFileStream( filePath.c_str(), std::ios::in );
    if ( !inputFileStream.good() )
    {
        THROW_RTE( "Failed to open file: " << filePath );
    }
