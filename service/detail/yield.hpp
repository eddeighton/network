...
struct yield_completion
{
    ...
    void wait()
    {
        // yield_handler_base::operator()() will set state_ `complete`
        // and attempt to wake a suspended fiber. It would be Bad if
        // that call happened between our detecting (complete !=
        // state_) and suspending.
        lock_t lk{ mtx_ };
        // If state_ is already set, we're done here: don't suspend.
        if( complete != state_ )
        {
            state_ = waiting;
            // suspend(unique_lock<spinlock>) unlocks the lock in the
            // act of resuming another fiber
...
public:
    ...
    void operator()( boost::system::error_code const& ec )
    {
        ...
        BOOST_ASSERT_MSG(
            ...
            "before calling yield_handler_base::operator()()" );
        // If originating fiber is busy testing state_ flag, wait
        // until it has observed (completed != state_).
        yield_completion::lock_t  lk{ ycomp_->mtx_ };
        yield_completion::state_t state = ycomp_->state_;
        // Notify a subsequent yield_completion::wait() call that it
        // need not suspend.
        ...
        *yt_.ec_ = ec;
        ...
        // immediately called its callback (this method!) before the
        // asio async function called async_result_base::get(), we
        // must not set it ready.
...
// specialize handler_type< yield_t, ... > to indicate
// yield_handler<>. So when you pass an instance of yield_t as an asio
// completion token, asio selects yield_handler<> as the actual
// handler class.
template < typename T >
...
public:
    ...
    void operator()( boost::system::error_code const& ec, T t )
    {
        ...
        // move the value to async_result<> instance BEFORE waking up
        // a suspended fiber
        *value_ = std::move( t );
    ...
    //  this must be injected by async_result before operator()() is
    //  called
    T* value_{ nullptr };
};
//]
...
// yield_handler<void> is like yield_handler<T> without value_. In
// fact it's just like yield_handler_base.
template <>
...
public:
        ...
        : ycomp_{ new yield_completion{} }
    {
        ...
        // if yield_t didn't bind an error_code, make
        // yield_handler_base's error_code* point to an error_code
        // local to this object so yield_handler_base::operator() can
        // unconditionally store through its error_code*
        if( !h.yt_.ec_ )
        {
            h.yt_.ec_ = &ec_;
        }
    ...
    void get()
    {
        // Unless yield_handler_base::operator() has already been
        // called, suspend the calling fiber until that call.
        ycomp_->wait();
        // The only way our own ec_ member could have a non-default
        // value is if our yield_handler did not have a bound
        // error_code AND the completion callback passed a non-default
        // error_code.
        if( ec_ )
        {
...
private:
    // If yield_t does not bind an error_code instance, store into
    // here.
...
// constructs the yield_handler, constructs this async_result
// specialization from it, then returns the result of calling its
// get() method.
...
public:
    ...
    using completion_handler_type
        = fibers::asio::detail::yield_handler< T >;

    explicit async_result(
        boost::fibers::asio::detail::yield_handler< T >& h )
        : boost::fibers::asio::detail::async_result_base{ h }
    {
        // Inject ptr to our value_ member into yield_handler<>:
...
public:
    ...
    using completion_handler_type
        = fibers::asio::detail::yield_handler< void >;

    explicit async_result(
        boost::fibers::asio::detail::yield_handler< void >& h )
        : boost::fibers::asio::detail::async_result_base{ h }
    {
    }
};
