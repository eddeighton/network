...
struct is_convertible< Pointer, to >
{
    static constexpr const bool value = true;
};

template < typename from >
struct is_convertible< from, Pointer >
{
    static constexpr const bool value = true;
};
...
{
    static inline bool test( const Pointer& from )
    {
        T check( from );
        return check;
    }
...
struct [[clang::eg_type( mega::id_Variant )]] __eg_variant : public mega::Pointer
{
    ...
    /*inline __eg_variant( const Pointer& from )
    {
    ...
    inline __eg_variant& operator=( const Pointer& from )
    {
        // when convert from event need to check runtime type against all __eg_variant types
        if ( ( !from.valid() ) || !mega::variant_runtime_type_check< Ts... >::test( from ) )
        {
